...
def create_app(webhook_secret: str | None = None, service: EcoPilotService | None = None) -> FastAPI:
    settings = Settings()
    ...
    if service is None:
        ...
        if settings.bigquery_table_id:
            sink = BigQuerySink(client=_build_bq_client(), table_id=settings.bigquery_table_id)
