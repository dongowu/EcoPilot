...
def build_setup_commands(env: dict[str, str]) -> list[str]:
    ...
    commands = [
        ...
        (
            ...
            f"--set-env-vars ECOPILOT_GCP_BILLING_TABLE_ID={project}.billing.gcp_export "
            f"--set-env-vars ECOPILOT_GCP_PROJECT_ID={project}"
        ),
...
def _parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--validate-env", action="store_true", help="Validate required env vars"
    )
    parser.add_argument(
        "--print-commands", action="store_true", help="Print setup commands"
    )
