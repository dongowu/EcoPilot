...
"""
    ...
    def add_mr_label(self, project_id: int, mr_iid: int, labels: list[str]) -> None:
        self.labels.append(
            {"project_id": project_id, "mr_iid": mr_iid, "labels": labels}
        )
    ...
    def list_mr_notes(self, project_id: int, mr_iid: int) -> list[dict]:
        return [
            ...
            if note["project_id"] == project_id and note["mr_iid"] == mr_iid
        ]
...
class SuccessLLM:
    def generate(self, prompt: str) -> str:
        if "Assess deployment risk" in prompt:
            return (
                "merge_confidence: medium\n"
                "rollback_note: Revert the generated MR if cache invalidation causes flaky builds.\n"
                "reasoning: Cache updates are usually safe but should be monitored."
            )
        return "AI summary"
...
def test_webhook_to_comment_and_bq() -> None:
    ...
    assert "Merge confidence: medium" in gitlab_client.notes[0]["body"]
    assert "Revert the generated MR" in gitlab_client.notes[0]["body"]
