...
def _sample_cloud_impact() -> CloudImpactSummary:
    return CloudImpactSummary(
        provider="gcp",
        estimated_monthly_cost_usd=240.0,
        estimated_monthly_savings_usd=36.0,
        estimated_annual_savings_usd=432.0,
        estimated_monthly_carbon_kgco2e=28.8,
...
def test_reporter_uses_llm_output_when_available() -> None:
    ...
    body, mode = reporter.render(
        _sample_context(), _sample_findings(), _sample_impact()
    )
...
def test_reporter_falls_back_when_llm_fails() -> None:
    ...
    body, mode = reporter.render(
        _sample_context(), _sample_findings(), _sample_impact(), _sample_cloud_impact()
    )
    assert mode == "fallback"
    assert "missing_cache" in body
    assert "estimated savings" in body.lower()
    assert "annualized impact" in body.lower()
    assert "recommended next step" in body.lower()
    assert "$31.2" in body
    assert "google cloud impact" in body.lower()
    assert "$432.0" in body
